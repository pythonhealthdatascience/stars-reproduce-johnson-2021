...
```{r}
original_sall_prep <- original_sall %>%
  # Replace S1NoCD with S0
  mutate(Scenario = replace(Scenario, Scenario == "S1NoCD", "S0")) %>%
  # Round columns
  mutate(CostpAgent = round(CostpAgent, 0)) %>%
  ...
  mutate(IncrementalNMB = round(IncrementalNMB, 0)) %>%
  # Select relevant columns
  select(Scenario, Interval, CostpAgent, QALYpAgent, ICER, IncrementalNMB)

# Append "_repo" to columns (except Scenario and Interval)
icol <- which(names(original_sall_prep) %in% c("Scenario", "Interval"))
colnames(original_sall_prep)[-icol] <- paste(colnames(original_sall_prep)[-icol], "repo", sep = "_")
...
calc_diff_perc <- function(df, our_res, paper_res, repo_res, prefix) {
  ...
  paperdiff <- paste0(prefix, "PaperDiff")
  paperperc <- paste0(prefix, "PaperPerc")
  ...
  df %>%
    mutate(
      !!sym(diff) := !!sym(our_res) - !!sym(paper_res),
      !!sym(paperperc) := scales::percent(round(!!sym(paperdiff) / !!sym(paper_res), 3)),
      !!sym(repodiff) := !!sym(our_res) - !!sym(repo_res),
    ...
    select(Scenario, Interval,
           !!sym(our_res), !!sym(paper_res), !!sym(repo_res),
           !!sym(paperdiff), !!sym(repodiff),
           !!sym(paperperc), !!sym(repoperc))
}

# Combine the model and paper results
sall_combine <- merge(sall_prep, paper_tab3_prep, by=c("Scenario", "Interval")) %>%
  merge(original_sall_prep, by=c("Scenario", "Interval"))

# Define the pairs of columns to compare and the prefix to use for their comparison
column_pairs <- list(
  c("CostpAgent", "CostpAgent_paper", "CostpAgent_repo", "Cost"),
  c("QALYpAgent", "QALYpAgent_paper", "QALYpAgent_repo", "QALY"),
  c("ICER", "ICER_paper", "ICER_repo", "ICER"),
  c("IncrementalNMB", "IncrementalNMB_paper", "IncrementalNMB_repo", "INMB")
)

# Combine model and paper results and apply the function for each pair of columns
sall_compare <- column_pairs %>%
  purrr::map(~ calc_diff_perc(sall_combine, .x[1], .x[2], .x[3], .x[4])) %>%
