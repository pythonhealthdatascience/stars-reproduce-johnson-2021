---
title: "Sensitivity analysis: 0.01 Treatment Utility"
...
```{r opts, echo = FALSE}
# CHANGE: Altered save path for image and change name of ceplot chunk below
...
write.csv(s1, paths$s1, row.names=FALSE)
```
...
write.csv(S2, paths$s2, row.names=FALSE)
```
...
write.csv(s3, paths$s3, row.names=FALSE)
```
...
write.csv(sall.tab, paths$sall, row.names=FALSE)
```
...
write.csv(ceplane, paths$ceplane, row.names=FALSE)
```


```{r Sen4_01_Treat_Util_CEplot_5y, echo=FALSE}
...
write.csv(clinical.results.tab, paths$clinical, row.names=FALSE)
```
